...
"""
    ...
    Update 25th of Nov 2019: previously this Bite required re-raising the KeyError, but as that's 
    already the default behavior of a missing key in a dict, we changed the requirements to return a value instead.
"""
...
def get_workout_motd(day):
    ...
    """
    try:
        if day.capitalize() in WORKOUT_SCHEDULE:
            print(day)
            return f'{TRAIN} {WORKOUT_SCHEDULE.get(day.capitalize())}'
        else:
            return INVALID_DAY
    except:
        return INVALID_DAY
